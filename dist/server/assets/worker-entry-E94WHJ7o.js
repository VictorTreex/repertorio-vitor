...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-wz5jVVI4.js").then((n) => n.a1).then(
      (m) => m.default ?? m
    );
  }
...
const server = {
  async fetch(request, env2, ctx) {
    try {
      ...
      normalizedResponse.headers.set("X-Frame-Options", "DENY");
      normalizedResponse.headers.set("X-Content-Type-Options", "nosniff");
      normalizedResponse.headers.set("X-XSS-Protection", "1; mode=block");
